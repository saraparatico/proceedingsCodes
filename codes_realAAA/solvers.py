...
class Coupled(object):
      ...
      def linear_run(self, T, dt, velocity_space, u_h, U_inlet, rho, nu, weakControl, nitsche_gamma, beta, theta, backflow,
            ...
            if not weakControl:
                        if self.problem.control == 'inlet':
                              bc_inlet = DirichletBC(W.sub(0), g, mf, inflow)
                              bc_walls = DirichletBC(W.sub(0), zero, mf, walls)
                              bcu = [bc_inlet, bc_walls]
                              bcp = []
                              if not tractionFree:
                                    bcp = DirichletBC(W.sub(1), Constant(0), mf, outflow)
...
class IPCS(object):
      ...
      def runIPCS(self, t_range, g, nu, rho, BDF2, stationary, dt, weakControl):
            ...
            for t in t_range[1:]:
                  ...
                  u_check = Function(V)
                  for bc in bcs_u:
                        bc.apply(u_check.vector())
                  XDMFFile("./check_bc.xdmf").write(u_check)
