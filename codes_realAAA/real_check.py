...
g = problem.construct_check_control()
...
with XDMFFile(MPI.comm_world, join(logDir, 'check/Check{}_Stationary{}Umax{}.xdmf'.format(patient, profile, ObsUmax))) as file:
...
error_squared = assemble(inner((u_h - u_obs), (u_h - u_obs)) * dx)
# Numero totale di nodi della mesh
Q = problem.V
num_nodes = len(Q.dofmap().cell_dofs(0))
# Calcola l'errore quadratico medio normalizzato
mean_squared_error = error_squared / num_nodes
# Calcola la radice quadrata dell'errore quadratico medio normalizzato
root_mean_squared_error = sqrt(mean_squared_error)
#error_rms = sqrt(error_squared)
